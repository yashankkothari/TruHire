...
async function initializeGeminiWithContext(profile = 'interview', language = 'en-US', candidateContext = '') {
    const apiKey = localStorage.getItem('apiKey')?.trim();
    if (apiKey) {
        // Combine custom prompt with candidate context
        const baseCustomPrompt = localStorage.getItem('customPrompt') || '';
        const combinedPrompt = baseCustomPrompt + '\n\n' + candidateContext;
        
        const success = await ipcRenderer.invoke('initialize-gemini', apiKey, combinedPrompt, profile, language);
        if (success) {
            cheddar.setStatus('Live - Interview Mode');
        } else {
            cheddar.setStatus('error');
        }
    }
...
// Listen for responses - REMOVED: This is handled in TruHireApp.js to avoid duplicates
    ...
    try {
        if (isMacOS) {
            ...
            if (audioMode === 'mic_only' || audioMode === 'both') {
                let micStream = null;
                try {
                    // Get selected microphone device or use default
            ...
            // Windows - use display media with loopback for system audio (stereo mix)
            // This captures audio from the interviewee's meeting application (Google Meet, Zoom, etc.)
            mediaStream = await navigator.mediaDevices.getDisplayMedia({
                video: {
                    frameRate: 1,
                    width: { ideal: 1920 },
            ...
            if (audioMode === 'mic_only' || audioMode === 'both') {
                let micStream = null;
                try {
                    // Capture interviewer's microphone audio separately
                    // Get selected microphone device or use default
                    const selectedMicId = localStorage.getItem('selectedMicrophoneId');
                    const audioConstraints = {
                        sampleRate: SAMPLE_RATE,
                    ...
                    console.log('Windows microphone capture started - INTERVIEWER AUDIO');
                    setupLinuxMicProcessing(micStream);
                } catch (micError) {
...
const truHireApp = document.querySelector('truhire-app');
...
const cheddar = {
    ...
    getCurrentView: () => truHireApp.currentView,
    getLayoutMode: () => truHireApp.layoutMode,

    // Status and response functions
    setStatus: text => truHireApp.setStatus(text),
    setResponse: response => truHireApp.setResponse(response),
